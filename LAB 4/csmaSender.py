...
def main():
    c = 0
    frame_to_send = "Hello, Arveus!".encode()
    ...
    while not is_transmission_successful:
        ...
        if not is_transmission_successful:
            print(
                f"Didn't recieve acknowledgement from reciever in the time bound.. trying again")
            c += 1
            if c > max_c:
                print(
                    f"Couldn't send data to the reciever.. backed off {max_c} times")
                break
